...
parser.add_argument('--save_model', type=str, default='best_model.pt', help='Path to save the best model')
parser.add_argument('--save_outputs', type=str, default='outputs.pt', help='Path to save all output tensors (softmax probabilities)')
...
def collect_all_outputs():
    """Collect all output logits for all nodes and return as tensor ordered by node ID."""
    model.eval()
    all_outputs = []
    ...
    for data in test_loader:
        data = transform(data)
        out = model(data.x, data.adj_t)
        # Store outputs (before argmax)
        all_outputs.append(out.cpu())
    
    # Concatenate all batches
    all_outputs = torch.cat(all_outputs, dim=0)  # [total_nodes, num_classes]
...
for epoch in range(1, args.epochs):
    ...
    if val_acc > best_val:
        ...
        print(f'  -> Saved best model (val_acc: {100 * best_val:.2f}%)')
...
# Load the best model and collect final output tensors
if best_epoch > 0:
    print(f'\nLoading best model from epoch {best_epoch}...')
    checkpoint = torch.load(args.save_model, map_location=device)
    model.load_state_dict(checkpoint['model_state_dict'])
    
    print('Collecting all output tensors for all nodes...')
    all_outputs = collect_all_outputs()
    
    # Save all output tensors (softmax probabilities) to .pt file
    torch.save(all_outputs, args.save_outputs)
    print(f'Saved all output tensors (shape: {all_outputs.shape}) to {args.save_outputs}')
    print(f'Output tensor contains softmax probabilities for all {all_outputs.shape[0]} nodes and {all_outputs.shape[1]} classes')
