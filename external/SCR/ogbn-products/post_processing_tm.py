...
def main(args):
    ...
    # -------------------------------
    # Save C&S predictions for downstream use
    # -------------------------------
    # y_soft is assumed to be [num_nodes, num_classes]
    save_base = f"./output/{args.dataset}/{args.file_name}_cs"

    # 1) Save as a PyTorch tensor (full logits / probabilities)
    torch.save(y_soft.cpu(), save_base + ".pt")
    print(f"Saved C&S predictions (tensor) to: {save_base}.pt")

    # 2) Optionally, save as NumPy for easy loading in other tools
    np.save(save_base + ".npy", y_soft.cpu().numpy())
    print(f"Saved C&S predictions (numpy) to: {save_base}.npy")

    # 3) Optionally, save hard labels (argmax) if useful
    hard_preds = y_soft.argmax(dim=1)
    np.save(save_base + "_argmax.npy", hard_preds.cpu().numpy())
    print(f"Saved C&S hard labels to: {save_base}_argmax.npy")
