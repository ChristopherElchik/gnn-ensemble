...
def main():
    ...
    parser.add_argument('--max_runs', type=int, default=None, help='Maximum number of runs to process (default: all)')
    ...
    # Sort to ensure consistent ordering (0.pt, 1.pt, 2.pt, ...)
    model_outs.sort(key=lambda x: int(os.path.splitext(os.path.basename(x))[0]))
    
    # Limit to first N runs if specified
    if args.max_runs is not None:
        model_outs = model_outs[:args.max_runs]
        print(f"Processing only first {len(model_outs)} run(s)")
    ...
    # Create predictions directory for final C&S predictions
    predictions_dir = f'predictions/{args.dataset}_{args.method}'
    os.makedirs(predictions_dir, exist_ok=True)
    
    while True:
        if args.method == 'plain':
            evaluate_params(data, eval_test, model_outs, split_idx, plain_dict, fn = plain_fn, save_dir=predictions_dir)
        elif args.method == 'linear':
            evaluate_params(data, eval_test, model_outs, split_idx, linear_dict, fn = linear_fn, save_dir=predictions_dir)
        elif args.method == 'mlp':
            evaluate_params(data, eval_test, model_outs, split_idx, mlp_dict, fn = mlp_fn, save_dir=predictions_dir)
        elif args.method == 'gat':
            evaluate_params(data, eval_test, model_outs, split_idx, gat_dict, fn = gat_fn, save_dir=predictions_dir) 
